...
Phases:
  ...
  4. Unit tests (pytest per service) → JUnit XML
  5. E2E HTTP scenarios
  6. Load test (k6)
  7. Collect docker stats (optional)
...
def phase_compose_up() -> PhaseResult:
    ...
    t0 = time.monotonic()
    r = _run(["docker", "compose", "-f", COMPOSE_FILE, "up", "-d", "--build", "--wait"], timeout=600)
    dur = (time.monotonic() - t0) * 1000
    ok = r.returncode == 0
    return PhaseResult(name="compose_up", passed=ok, duration_ms=dur,
...
def phase_wait_ready() -> PhaseResult:
    ...
    dur = (time.monotonic() - t0) * 1000
    if failures:
        return PhaseResult(name="readiness", passed=False, duration_ms=dur,
                           detail=f"NOT READY: {', '.join(failures)}")
    return PhaseResult(name="readiness", passed=True, duration_ms=dur,
...
def phase_lint() -> PhaseResult:
    ...
    dur = (time.monotonic() - t0) * 1000
    if errors:
        return PhaseResult(name="lint", passed=False, duration_ms=dur,
                           detail=f"{len(errors)} lint error(s)",
                           sub_results=[{"error": e} for e in errors[:10]])
    return PhaseResult(name="lint", passed=True, duration_ms=dur, detail="ruff + black + mypy OK")


def phase_unit_tests() -> PhaseResult:
    """Run pytest per service, collect JUnit XML."""
    _log("🧪", "Running unit tests …")
    t0 = time.monotonic()
    os.makedirs(ARTIFACTS_DIR, exist_ok=True)
    junit_path = os.path.join(ARTIFACTS_DIR, "junit.xml")
    failures: list[str] = []
    total_passed = 0
    total_failed = 0
    ...
    for svc in SERVICES:
        ...
        r = _run(
            ...
            cwd=svc_dir, timeout=300,
        )
        if r.returncode != 0:
            failures.append(f"{svc}: {r.stdout[-200:]}")
            total_failed += 1
        else:
            total_passed += 1

    dur = (time.monotonic() - t0) * 1000
    detail = f"{total_passed}/{len(SERVICES)} services passed"
    if failures:
        return PhaseResult(name="unit_tests", passed=False, duration_ms=dur,
                           detail=detail,
                           sub_results=[{"error": e} for e in failures])
    return PhaseResult(name="unit_tests", passed=True, duration_ms=dur, detail=detail)
...
def phase_e2e() -> PhaseResult:
    ...
    try:
        ...
        return PhaseResult(name="e2e_http", passed=False,
                           duration_ms=(time.monotonic() - t0) * 1000,
                           detail=str(exc))

    sub = [
        {"step": s.name, "passed": s.passed, "duration_ms": s.duration_ms, "detail": s.detail}
    ...
    return PhaseResult(
        name="e2e_http", passed=e2e_report.passed, duration_ms=dur,
...
def phase_load() -> PhaseResult:
    ...
    if not shutil.which("k6"):
        return PhaseResult(name="load_test", passed=True,
                           duration_ms=(time.monotonic() - t0) * 1000,
    ...
    r = _run(
        ["k6", "run", "--out", f"json={ARTIFACTS_DIR}/k6-results.json",
         K6_SCRIPT],
        timeout=1800,
    )
...
def phase_docker_stats() -> int:
    """Sample container restarts. Returns total restart count."""
    _log("📊", "Checking container restarts …")
    try:
        r = _run(["docker", "compose", "-f", COMPOSE_FILE, "ps", "--format", "json"], timeout=30)
        if r.returncode != 0:
            return 0
        restarts = 0
        # docker compose ps --format json returns one JSON per line
...
def phase_compose_down() -> PhaseResult:
    ...
    t0 = time.monotonic()
    r = _run(["docker", "compose", "-f", COMPOSE_FILE, "down", "-v"], timeout=120)
    dur = (time.monotonic() - t0) * 1000
    return PhaseResult(name="compose_down", passed=r.returncode == 0, duration_ms=dur,
...
def main() -> int:
    ...
    p = phase_wait_ready()
    report.add_phase(p)
    if not p.passed and not args.fast:
    ...
    # ── 4. Unit tests ─────────────────────────────────────────
    if not args.e2e_only and not args.load_only:
        report.add_phase(phase_unit_tests())
    ...
    # ── 7. Docker stats ──────────────────────────────────────
    report.container_restarts = phase_docker_stats()
